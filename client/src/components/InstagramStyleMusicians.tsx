...
interface LiveInstagramProfile {
...
const MusicianCard = ({ musician, index }: MusicianCardProps) => {
  ...
  const username = musician.handle.replace(/^@/, "");

  const { data: liveProfile } = useQuery<LiveInstagramProfile>({
    queryKey: [`/api/instagram/profile?username=${encodeURIComponent(username)}`],
  });

  const followers = liveProfile?.followers || musician.followers;
  const profileImage = liveProfile?.available ? liveProfile.profileImage : null;
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-4 text-sm text-muted-foreground">
              ...
              <span><strong className="text-card-foreground">{followers}</strong> seguidores</span>
